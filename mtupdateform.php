...
<?php
	...
	<?php 
		...
		$sqlUpdate = "SELECT * FROM web_content WHERE type_id = '$articleID' AND type = '$type'";
	...
	<div class="container">
		...
		<form action="lib/updateContent.php" method="post" id="mtForm" enctype="multipart/form-data">	
			<label>Title</label>
			<input required type="text" name="title" class="form-control mb-3" placeholder="Title" value="<?php echo $res['title'] ?>">
			<input type="text" hidden name="type" value="<?php echo $type; ?>">
			<input type="text" hidden name="articleID" value="<?php echo $articleID; ?>">
			<?php if ($type == 'MtCrimeSiteRecommendation'): ?>
				<label>Domain Name</label>
				<input required type="text" class="form-control" name="domain-name" value="<?php echo $res['domainName'] ?>">
				<label>Code</label>
				<input required type="text" class="form-control" name="web-code" value="<?php echo $res['code'] ?>">
				<label>Deposit</label>
				<input required type="text" class="form-control" name="deposit" value="<?php echo $res['deposit'] ?>">
				<label>Description</label>
				<input required type="text" class="form-control" name="description" value="<?php echo $res['description'] ?>">
			<?php endif ?>
			...
			<label>Content</label>
			<textarea required name="content" id="addText"><?php echo $res['content'] ?></textarea>
